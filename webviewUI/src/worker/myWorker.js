var workerLib = (() => {
  ...
  // src/worker/csgChange.ts
  var csgChange_exports = {};
  __export(csgChange_exports, {
    getCsgObjArray: () => getCsgObjArray
  });
  var geometries = {
    geom3: { isA: (object) => {
  ...
  var getCsgObjArray = (db, back) => {
    try {
      ...
      for (let i = 0; i < db.length; i++) {
        back({ ver: getCsgObj(db[i], back), index: i });
      }
      back({ end: true });
    } catch (e) {
  ...
  return __toCommonJS(csgChange_exports);
})();
