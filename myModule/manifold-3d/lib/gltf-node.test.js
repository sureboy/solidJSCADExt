...
import { BaseGLTFNode, CrossSectionGLTFNode, GLTFNode, GLTFNodeTracked, VisualizationGLTFNode } from "./gltf-node.js";
...
suite('Ensure manifold and glTF rotations match', () => {
    ...
    const pts = [[-1, 0, 0], [4, 0, 0], [0, -2, 0], [0, 3, 0], [0, 0, 5]];
    const getManifoldScript = (rotX, rotY, rotZ) => `import {Manifold} from \'manifold-3d/manifoldCAD\';\n` +
        `export default Manifold\n` +
        `  .hull(${JSON.stringify(pts)})\n` +
        `  .rotate(${rotX}, ${rotY}, ${rotZ});\n`;
    const getGltfScript = (rotX, rotY, rotZ) => `import {Manifold, GLTFNode} from \'manifold-3d/manifoldCAD\';\n` +
        `const node = new GLTFNode();` +
        `node.manifold = Manifold\n` +
        `  .hull(${JSON.stringify(pts)});\n` +
        `node.rotation = [${rotX}, ${rotY}, ${rotZ}];\n` +
        `export default node;`;
    const getPoints = async (script) => {
...
suite('anyToGLTFNodeList()', () => {
    ...
    test('Throws when .crossSection is empty', async () => {
        const script = `import {CrossSectionGLTFNode} from 'manifold-3d/manifoldCAD';\n` +
...
//# sourceMappingURL=gltf-node.test.js.map