...
import { gltfDocToManifold, importModel } from "./import-model.js";
import { cleanup, evaluate, exportBlobURL } from "./worker.js";
async function resolveExample(name) {
    const [filepath] = await glob(name.toLowerCase().replaceAll(' ', '-') + '.{ts,mjs,js}', {
...
async function runExample(name, firstMeshOnly = true) {
    ...
    const firstMesh = document.getRoot().listNodes().find(node => !!node.getMesh());
    const manifold = await (firstMeshOnly ? gltfDocToManifold(document, firstMesh) :
