...
import { equalsVec3Array, meshToVec3Array } from "../test/util.js";
import { toArrayBuffer } from "./export-model.js";
import * as importer from "./import-model.js";
...
suite('importManifold()', () => {
    test('imports a model with EXT_mesh_manifold', async () => {
        const model = await importer.importManifold(new URL('../test/fixtures/models/boxExtMeshManifold.glb', import.meta.url));
        expect(model.volume()).to.be.closeTo(100 * 100 * 100, 1);
    });
    test('imports a model without EXT_mesh_manifold', async () => {
        const model = await importer.importManifold(new URL('../test/fixtures/models/box.glb', import.meta.url));
        expect(model.volume()).to.be.closeTo(100 * 100 * 100, 1);
    });
    test('throws when model is not manifold', async () => {
        const fn = async () => await importer.importManifold(new URL('../test/fixtures/models/boxNotManifold.glb', import.meta.url));
        await expect(fn).rejects.toThrowError();
    });
    test.skip('succeeds when tolerance permits a non-manifold model', async () => {
        const model = await importer.importManifold(new URL('../test/fixtures/models/boxNotManifold.glb', import.meta.url), { tolerance: 0.01 });
        expect(model.volume()).to.be.closeTo(100 * 100 * 100, 1);
    });
    test('throws when tolerance is insufficient', async () => {
        const fn = async () => await importer.importManifold(new URL('../test/fixtures/models/boxNotManifold.glb', import.meta.url), { tolerance: 0.001 });
        await expect(fn).rejects.toThrowError();
    });
    ...
    test('import a model by absolute path', async () => {
        const model = await importer.importManifold(import.meta.dirname + '/../test/fixtures/models/box.glb');
        expect(model.volume()).to.be.closeTo(100 * 100 * 100, 1);
    });
    test('import a model from a Blob (with mimetype)', async () => {
        const buffer = await fs.readFile(import.meta.dirname + '/../test/fixtures/models/box.glb');
        const blob = new Blob([buffer.buffer], { type: 'model/gltf-binary' });
    ...
    test('import a model from a Blob (specifying mimetype)', async () => {
        ...
        const model = await importer.importManifold(blob, { mimetype: 'model/gltf-binary' });
        expect(model.volume()).to.be.closeTo(100 * 100 * 100, 1);
    });
    test('import a model from an ArrayBuffer', async () => {
        const buffer = await fs.readFile(import.meta.dirname + '/../test/fixtures/models/box.glb');
        const model = await importer.importManifold(buffer.buffer, { mimetype: 'model/gltf-binary' });
        expect(model.volume()).to.be.closeTo(100 * 100 * 100, 1);
    });
    test('Orients a model correctly', async () => {
        ...
        const before = [[-1, 0, 0], [4, 0, 0], [0, -2, 0], [0, 3, 0], [0, 0, 5]];
...
suite('importModel()', () => {
    test('uses source filename when imported node has no name', async () => {
        const node = await importer.importModel(new URL('../test/fixtures/models/box.glb', import.meta.url));
        expect(node.name).toBe('box.glb');
    });
    test('prefers source node name when present', async () => {
        const node = await importer.importModel(new URL('../test/fixtures/models/boxNotManifold.glb', import.meta.url));
        expect(node.name).toBe('obj1');
    });
});
//# sourceMappingURL=import-model.test.js.map